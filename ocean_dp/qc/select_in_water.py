...
def select_in_water(netCDFfiles):
    ...
    for fn in netCDFfiles:
        ...
        if os.path.basename(fn).split('_')[0]=='IMOS':        
        ...
        try:
            ...
            hist = ""        
        ds.history += hist + now.strftime("%Y-%m-%d : ") + 'Data subset to only contain deployed (in water) data - the full record can be found in the corresponding FV00 file ' + os.path.basename(fn) + '.'
