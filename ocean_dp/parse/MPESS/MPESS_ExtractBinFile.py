...
# v4.0 - Match firmware ver 4.x
# v4.0 - Change Data structure to include sensor searial numbers
# v4.0 - Change Data structure to include Battery voltage reading at end
# v4.1 - Change Info data structure to include Platform
...
if (Flag_PlotData == False):    Flag_SavePlots = False

# logger
#DeploymentFilePath = r'/home/parallels/Desktop/Parallels Shared Folders/Home/CSIRO/Projects/0201 MPESS/Logger v1/Data/ver4/MPESS_U%02u_D%03u' % (UnitNr, DeploymentNr)


# Mini-Logger
# DeploymentFilePath = r'/home/parallels/Desktop/Parallels Shared Folders/Home/CSIRO/Projects/0201 MPESS/MiniLogger v1/Data/v4/MPESS_U%02u_D%03u' %(UnitNr,DeploymentNr)
#
# if (UnitNr == 1):   DeploymentFileName = 'MPESS_U01_S24C6C99A_D%03u_Data.bin' % DeploymentNr
# elif (UnitNr == 2): DeploymentFileName = 'MPESS_U02_SC6D2EBFC_D%03u_Data.bin' % DeploymentNr
# elif (UnitNr == 5): DeploymentFileName = 'MPESS_U05_SEDD6B5B1_D%03u_Data.bin' % DeploymentNr
# elif (UnitNr == 6): DeploymentFileName = 'MPESS_U06_SCE5C93B7_D%03u_Data.bin' % DeploymentNr
# else:               DeploymentFileName = 'unknown.bin'

DeploymentFilePath = os.path.dirname(sys.argv[1])
DeploymentFileName = os.path.basename(sys.argv[1])
...
def ExtractAllSamples(deployment_file, file_Info, writer_CSV):
    ...
    # create CSV header
    if Flag_CreateCSV:
        HeaderList = ['Time (UTC)', 'Battery Voltage (Start) [V]', 'Battery Voltage (End) [V]', 
                      'Pressure [dbarA]', 'LineForce [N]',
                      'IMU_TimeStamp [ms]',  
    ...
    while not Flag_AllDone:
        ...
        if BytesDeleted:
            DebugString = 'Corrupted Data Deleted (%u Bytes)' % BytesDeleted
        ...
        if not Flag_AllDone:
            ...
            # -------------------------
            # Noraml Sample
            if (SampleType & 0xFFFFFFF0) == MPS.SAMPLE_TYPE_NORMAL:
            ...
            # -------------------------
            # Intensive Sample
            elif (SampleType & 0xFFFFFFF0) == MPS.SAMPLE_TYPE_INTENSIVE:
            ...
            # -------------------------
            # Info
            elif SampleType == MPS.SAMPLE_TYPE_INFO:
...
def ScanForNextSampleType(deployment_file):
    ...
    while True:
        ...
        if ((SampleType & 0xFFFFFFF0) == MPS.SAMPLE_TYPE_NORMAL) or \
                ((SampleType & 0xFFFFFFF0) == MPS.SAMPLE_TYPE_INTENSIVE) or \
                (SampleType == MPS.SAMPLE_TYPE_INFO):
...
def ProcessSample_Intensive(deployment_file, writer_CSV):
    ...
    sample_size = ctypes.sizeof(MPS.Struct_Sample_Start)

    class SampleUnion(ctypes.Union):  # must fill blocks of 4 bytes
        _fields_ = [("Data", MPS.Struct_Sample_Start),
    ...
    data['BatteryVoltage_Start'] = UN.Data.BatteryVoltage_Start
    data['BatteryVoltage_End'] = np.NaN   
    if SensorsPresent & MPS.SNS_MASK_IMU:
    ...
    if SensorsPresent & MPS.SNS_MASK_LT:
        data['LineForce'] = np.NaN
    if SensorsPresent & MPS.SNS_MASK_PT:
        data['Pressure'] = np.NaN
    data_lists.append(data)  # append
    if Flag_CreateCSV:
        BattSt = '%.3f' %data['BatteryVoltage_Start']
        writer_CSV.writerow(
            (DateTimeUtc.strftime('%Y/%m/%d %H:%M:%S'), BattSt, '', '', '', '', '', '', ''))

    # ------------------------------
    # read recursive part of sample
    sample_size = 4;                                                       # SamplesToFollow
    if SensorsPresent & MPS.SNS_MASK_LT:   sample_size += 4                # Line Force
    if SensorsPresent & MPS.SNS_MASK_IMU:  sample_size += 4*(1+3+3+3+9)    # IMU (TimeStampMs+Accelerometer+Gyro+Magnetometer+OrientationMatrix)
    if SensorsPresent & MPS.SNS_MASK_PT:   sample_size += 4                # Pressure
    TimeDelta = datetime.timedelta(seconds=1 / MPS.sample_frequency)
    while True:
        ...
        data = {}
        data['Time'] = DateTimeUtc.astimezone(TimezoneLocal)
        data['BatteryVoltage_Start'] = np.NaN
        data['BatteryVoltage_End'] = np.NaN   
        SamplesToFollow = struct.unpack('>L', sample_buffer[0:4])[0]  # read data into union
        sample_buffer = sample_buffer[4:]
        if SensorsPresent & MPS.SNS_MASK_LT:  # Line Force
            data['LineForce'] = struct.unpack('>f', sample_buffer[0:4])[0]
            sample_buffer = sample_buffer[4:]
        if SensorsPresent & MPS.SNS_MASK_IMU:  # IMU
        ...
        if SensorsPresent & MPS.SNS_MASK_PT:  # Pressure
        ...
        # all done
        if SamplesToFollow == 0:    break

        # increse timestamp
        DateTimeUtc += TimeDelta

    # ------------------------------
    # read end-section single-shot part of sample
    ...
    data['Time'] = DateTimeUtc.astimezone(TimezoneLocal)
    data['BatteryVoltage_Start'] = np.NaN
    data['BatteryVoltage_End']   = struct.unpack('>f', sample_buffer[0:4])[0]
    if SensorsPresent & MPS.SNS_MASK_IMU:
        data['IMU_TimeStampMs'] = np.NaN
        data['IMU_AccX']        = np.NaN
        data['IMU_AccY']        = np.NaN
    ...
    data_lists.append(data)  # append
    if Flag_CreateCSV:
        BattSp = '%.3f' %data['BatteryVoltage_End']
        writer_CSV.writerow((DateTimeUtc.strftime('%Y/%m/%d %H:%M:%S'), '', BattSp, '', '', '', '', '', ''))
...
def ProcessSample_Normal(deployment_file, writer_CSV):
    ...
    # check checksum
        #calc sample size
    sample_buffer = deployment_file.read(4)     #read type
    SensorsPresent = (sample_buffer[3])
    sample_size_start = ctypes.sizeof(MPS.Struct_Sample_Start)
    if SensorsPresent & MPS.SNS_MASK_PT:   sample_size_start += 4
    sample_size_cont = 0
    if SensorsPresent & MPS.SNS_MASK_LT:   sample_size_cont += 4            # Line Force
    if SensorsPresent & MPS.SNS_MASK_IMU:  sample_size_cont += 4*(1+3+3+3+9)    # IMU (TimeStampMs+Accelerometer+Gyro+Magnetometer+OrientationMatrix)
    sample_size_end = 4         # battery voltage
    if SensorsPresent & MPS.SNS_MASK_PT:   sample_size_end += 4
    sample_size = 4 + sample_size_start + NrOfSamplesPerStop*sample_size_cont + sample_size_end + 4
    ...
    UN = SampleUnion()
    for sample_byte_index in range(sample_size_start):
        UN.Buffer[sample_byte_index] = (sample_buffer[4 + sample_byte_index])
    sample_buffer = sample_buffer[4 + sample_size_start:]
    DateTimeUtc = datetime.datetime.fromtimestamp(UN.Data.StartTime, tz=pytz.UTC)  # convert utc seconds to utc date and time
    data = {}
    data['Time'] = DateTimeUtc.astimezone(TimezoneLocal)
    data['BatteryVoltage_Start'] = UN.Data.BatteryVoltage_Start
    ...
    if Flag_CreateCSV:
        writer_CSV.writerow(
            (DateTimeUtc.strftime('%Y/%m/%d %H:%M:%S'), '%.1f' %data['BatteryVoltage_Start'], '', '', '', '', '', ''))
    ...
    if SensorsPresent & MPS.SNS_MASK_PT:
        ...
        if SensorsPresent & MPS.SNS_MASK_LT:
            data['LineForce'] = np.NaN
        if SensorsPresent & MPS.SNS_MASK_PT:
            data['Pressure'] = struct.unpack('>f', sample_buffer[4:8])[0]
        data_lists.append(data)  # append
        if Flag_CreateCSV:
            BattSp = '%.3f' % data['BatteryVoltage_End']
            if SensorsPresent & MPS.SNS_MASK_PT:
                PtSp = '%.3f' % data['Pressure']
            else:
                PtSp = ''
            writer_CSV.writerow((DateTimeUtc.strftime('%Y/%m/%d %H:%M:%S.%f')[:-5], '', BattSp, PtSp, '', '', '', '', ''))
        sample_buffer = sample_buffer[4:]
...
def ProcessSample_Info(deployment_file):
    ...
    sample_size = ctypes.sizeof(MPS.Struct_Sample_Info)

    class SampleUnion(ctypes.Union):  # must fill blocks of 4 bytes
        _fields_ = [("Data", MPS.Struct_Sample_Info),
                    ("Buffer", ctypes.c_uint8 * sample_size)]
    ...
    UnitNr = UN.Data.Sys_UnitNumber
    DeploymentNr = UN.Data.Dep_Number
    ...
    if UN.Data.Sys_Platform == MPS.SYS_PLATFORM_LOGGER:     InfoString += '  Platform = Logger\r\n'
    elif UN.Data.Sys_Platform == MPS.SYS_PLATFORM_MINI:     InfoString += '  Platform = Mini-Logger\r\n'
    else:                                                   InfoString += '  Platform = Unknown\r\n'
    ...
    InfoString += '  Deployment Number   = %u\r\n' % UN.Data.Dep_Number
    ...
    InfoString += '  Intensive Sampling Time = %u min (%lu Samples @ %.0fsps)\r\n' % (
                                UN.Data.Dep_Int_NrOfSamples / MPS.sample_frequency / 60,
                                UN.Data.Dep_Int_NrOfSamples, MPS.sample_frequency)
    InfoString += '  Normal Sampling Time    = %u min  (%lu Sample @ %.0fsps)\r\n' % (
                                UN.Data.Dep_Norm_NrOfSamples / MPS.sample_frequency / 60, 
                                UN.Data.Dep_Norm_NrOfSamples, MPS.sample_frequency)
    ...
    if UN.Data.Sns_SensorsPresent & 0b010:      SensorsPresentString += 'LoadCell, '
    ...
    if UN.Data.Sns_SensorsPresent & 0b100:  # IMU
        if UN.Data.Sns_IMU_Model == MPS.SNS_IMU_3DM_GX3_25:
            InfoString += '  IMU Model  = Microstrain 3DM-GX3-25\r\n'
            InfoString += '  IMU Serial = 6223-%04.0f.%05.0f\r\n' %(UN.Data.Sns_IMU_Serial//1e5, UN.Data.Sns_IMU_Serial%1e5)
        elif UN.Data.Sns_IMU_Model == MPS.SNS_IMU_3DM_GX4_25:
            InfoString += '  IMU Model = Microstrain 3DM-GX4-25\r\n'
            InfoString += '  IMU Serial = 6234-%04.0f.%05.0f\r\n' %(UN.Data.Sns_IMU_Serial//1e5, UN.Data.Sns_IMU_Serial%1e5)
        else:
            InfoString += '  IMU Model = Unknown\r\n'
    if UN.Data.Sns_SensorsPresent & 0b010:  # Load Cell
        InfoString += '  Load Cell Serial = PTv%04X\r\n' %UN.Data.Sns_LC_Serial
    if UN.Data.Sns_SensorsPresent & 0b001:  # pressure
        PressureUnits = ''
        index = 0
        while (index < 8) and (UN.Data.PT_CalUnits[index] != 0):
            PressureUnits += chr(UN.Data.PT_CalUnits[index])
            index += 1
...
def PlotDeploymentData(df):
    ...
    # ------------------------------
    # line Force
    if 'LineForce' in df.columns.values.tolist():
        plt.figure(FigNr, figsize=(24, 12))
        FigNr += 1
        plt.clf()
        df.LineForce.plot(style='-')
        plt.grid()
        plt.xlabel('Time (Local)')
        if DeploymentDuration.days > 0:
            plt.gcf().autofmt_xdate()
            plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%y/%m/%d %H:%M'))
        plt.ylabel('Line Force [N]')
        plt.title('Line Force during Deployment %u (Unit %u)' % (DeploymentNr, UnitNr))
        plt.tight_layout()
        plt.show()
        if Flag_SavePlots == True:
            plt.savefig(os.path.join(DeploymentFilePath, 'Line Force.png'), bbox_inches='tight', dpi=300)

    # ------------------------------
    # battery voltage
    if 'BatteryVoltage_Start' in df.columns.values.tolist():
        plt.figure(FigNr, figsize=(24, 12))
        FigNr += 1
        plt.clf()
        df.BatteryVoltage_Start[df.BatteryVoltage_Start.notnull()].plot(style='b-')
        df.BatteryVoltage_Start.plot(style='b.')
        df.BatteryVoltage_End[df.BatteryVoltage_End.notnull()].plot(style='r-')
        df.BatteryVoltage_End.plot(style='r.')
        plt.ylim(7, 17)
        plt.grid()
        plt.xlabel('Time (Local)')
        if DeploymentDuration.days > 0:
...
if not os.path.exists(bin_path_filename):
    ...
    writer_CSV = []
    if Flag_CreateCSV:
        FilenameCSV = DeploymentFileName.split('.')[0] + '.csv'
    ...
    FilenameInfo = DeploymentFileName.split('Data')[0] + 'Info.txt'
    ...
    file_Info.close()
    if Flag_CreateCSV:   file_CSV.close()
