...
def parse_file(files):
    ...
    for filepath in files:
        ...
        with open(filepath, "rb") as binary_file:
            ...
            for pkt_id in pkt_count:
                a = pkt_pos[pkt_id]
                print('id=', pkt_id, "'"+packet_decoder[pkt_id]['name']+"'", 'count=', pkt_count[pkt_id], len(a), a[0], 'len=', pkt_len[pkt_id])
                unpack = packet_decoder[pkt_id]['unpack']
                keys = packet_decoder[pkt_id]['keys']

                binary_file.seek(a[0])
                ...
                if packet_id['Vector Velocity Data Header'] == pkt_id:
                    ...
                    var_list = []
                    var_list.append((noise1, d['noise1'], 1, np.empty([number_samples], dtype=np.int8)))
                    var_list.append((noise2, d['noise2'], 1, np.empty([number_samples], dtype=np.int8)))
                    var_list.append((noise3, d['noise3'], 1, np.empty([number_samples], dtype=np.int8)))

                    var_list.append((corr1, d['corr1'], 1, np.empty([number_samples], dtype=np.int8)))
                    var_list.append((corr2, d['corr2'], 1, np.empty([number_samples], dtype=np.int8)))
                    var_list.append((corr3, d['corr3'], 1, np.empty([number_samples], dtype=np.int8)))
                    ...
                    ana2_array = np.empty([number_samples, number_data_samples])
                    pres_array[:] = np.NaN
                ...
                if packet_id['HR Aquadopp Profiler Velocity Data'] == pkt_id:
                    ...
                    imu_vec_list.append((vel3, d['vel['+str(cells*2)+']'], 1000, np.empty([number_samples, cells], dtype=np.float32)))
        ...
        for a in attribute_list:
            print('attribute: ', a)
            ncOut.setncattr('instrument_setup_'+a[0], a[1])
