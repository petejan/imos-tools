...
phyto = pd.read_csv(fn, on_bad_lines='warn', engine='python', encoding='ISO-8859-1')
...
genus = phyto.groupby(['Genus Name'])
species = phyto.groupby(['Species Name'])
taxon = phyto.groupby(['Taxon Name'])
taxon_group = phyto.groupby(['Taxon Eco Group'])
...
date_time = [datetime.strptime(x, '%d-%b-%y') for x in phyto['Sample Date']]
...
nc_cell_Out[:] = phyto['Cell Per Litre'].values
...
nc_bv_out[:] = phyto['Biovolume Um3 Per L'].values
...
aphia_id = phyto['Aphia Id'].values
