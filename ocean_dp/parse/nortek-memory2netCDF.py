                ...
                if checksum & 0xffff != (struct.unpack("<H", packet[-2:]))[0]:
                    ...
                    try:
                        ...
                        if 'Vector and Vectrino Probe Check Data' == packet_decoder[id]['name']:
                            number_samples = struct.unpack('<H', packet[0:2])[0]
                            print('Vector and Vectrino Probe Check Data, number samples', number_samples)
                            unpack = unpack.format(number_samples)

                        packetDecode = struct.unpack(unpack, packet)
                        packets_decoded += 1

                        #print('packet', id, packetDecode)
                        #print('packet id', id)

                        if 'Head Configuration' == packet_decoder[id]['name']:
                            number_beams = packetDecode[6]
                            print('Head Configuration, number beams', number_beams)

                        packet_data[id].append((id, packetDecode))

                    except KeyError:
                        print('packet_decode not found ', id)

                    if packets_decoded > 1000: # temporary, decode small section
                        break
...
#
    ...
    for i in range(0,127):
        if (len(packet_data[i])> 0):
            print(i, packet_decoder[i]['name'], len(packet_data[i]))

    print('aquadopp velocity data samples ', len(packet_data[1]))
    print('aquapro velocity data samples ', len(packet_data[33]))
    print('vector velocity data samples ', len(packet_data[16]))
    print('vector IMU data samples ', len(packet_data[113]))

    number_samples_read = len(packet_data[1]) + len(packet_data[33]) + len(packet_data[16])
