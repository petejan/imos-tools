...
def smooth(files):
    ...
    for filepath in files:
        ...
        fn_new = filepath.replace('.nc', '-binned-5dbar.nc')
        ...
        for var_name in ['TEMP', 'PSAL', 'DOX2']:
            ...
            for n in set(profile):
                ...
                y = np.full_like(d, np.nan, dtype=np.double)
                temp_profile = temp[msk]
                pres_profile = pres[msk]
                for i in range(1, len(d)-1):
                    y[i] = np.mean(temp_profile[(pres_profile > d[i-1]) & (pres_profile < d[i+1])])
        ...
        #  create history
        ds_new.history += '\n' + now.strftime("%Y-%m-%d : ") + 'binned data created from ' + os.path.basename(filepath) + ' window=' + str(window) + ' degree=' + str(degree)
