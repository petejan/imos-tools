...
# curl -o bgc_phytoplankton_abundance_raw_data.json "https://geoserver-123.aodn.org.au/geoserver/ows?service=WFS&version=1.1.0&request=GetFeature&typeName=imos:bgc_phytoplankton_abundance_raw_data&outputFormat=json"
# curl -o bgc_phytoplankton_biovolume_raw_data.json "https://geoserver-123.aodn.org.au/geoserver/ows?service=WFS&version=1.1.0&request=GetFeature&typeName=imos:bgc_phytoplankton_biovolume_raw_data&outputFormat=json"


cur = con.cursor()
cur.execute("CREATE TABLE IF NOT EXISTS bgc_phytoplankton_abundance_raw_data ("
            "FID VARCHAR(64),"
...
cur.execute("CREATE TABLE IF NOT EXISTS bgc_phytoplankton_biovolume_raw_data ("
            "FID VARCHAR(64),"
            "IMOS_SITE_CODE VARCHAR(50),"
            "SOTS_YEAR INTEGER,"
            "SOTS_DEPLOYMENT VARCHAR(50),"
    ...
    for f in data["features"]:
        if f['properties']['Project'] == 'SOTS':
            ...
            for a in adb_data:
                if adb_data[a] != 0:
                    print(a, adb_data[a])
                    cur.execute("INSERT INTO bgc_phytoplankton_abundance_raw_data ("
                                "FID, "
                                "IMOS_SITE_CODE, "
                                "SOTS_YEAR, "
                                "SOTS_DEPLOYMENT, "
    ...
    for f in data["features"]:
        if f['properties']['Project'] == 'SOTS':
            ...
            for a in adb_data:
                if adb_data[a] != 0:
                    print(a, adb_data[a])
                    cur.execute("INSERT INTO bgc_phytoplankton_biovolume_raw_data ("
                                "FID, "
                                "IMOS_SITE_CODE, "
                                "SOTS_YEAR, "
                                "SOTS_DEPLOYMENT, "
